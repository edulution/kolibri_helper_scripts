...
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
  ...
  echo
  echo "DESCRIPTION"
  echo "  Inserts subscription for each classroom and learner group using the channel "
  echo "  subscriptions sql script."
  echo 
  echo "Example"
...
-a -f ~/.kolibri_helper_scripts/sql/channel_subscriptions.sql
