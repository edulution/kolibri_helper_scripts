...
FOR classRow IN (SELECT *FROM kolibriauth_collection WHERE kind = 'classroom') LOOP 
	...
	-- ELSE
		...
		WHERE kind = 'classroom'
		 ...
		 /*Subscriptions for each level*/ /*Each group corresponds to 1 level and can see all the sections for that level and all preceeding levels */
		UPDATE kolibriauth_collection
