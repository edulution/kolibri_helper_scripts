...
def create_users(input_file):
	with open(input_file) as f:
	    ...
	    for user in users:
	    	...
	    	FacilityUser.objects.create(id=new_user_id,full_name=user['full_name'],username=user['username'],password=make_password(user['username']),dataset_id=dataset_id,facility_id=def_facility,_morango_partition = _morango_partition, _morango_source_id = uuid.uuid4())
