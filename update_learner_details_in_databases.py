...
def update_learner_details(input_file):
        ...
        with open(input_file) as f:
            ...
            for user in users:
                ...
                if not username.strip() or not full_name.strip():
                    print_colored("Skipping user with ID {}: Username or Full name is missing or empty".format(user_id), colors.fg.red)
                    skipped_count += 1
                    continue
                ...
                try:
                    ...
                    print_colored("User with ID {}: Does not exist".format(user_id), colors.fg.red)
                    continue

                # Check for duplicate username in the database
                if FacilityUser.objects.exclude(id=user_id).filter(username=username).exists():
                    print_colored("Duplicate username. There is already a user called {}".format(username), colors.fg.red)
                    continue

                # Check if user details have changed and update them
                if (user_object.full_name == full_name) and (user_object.username == username):
                    unchanged_count += 1
                    print_colored("User with ID {}: Details have not changed".format(user_id), colors.fg.yellow)
                else:
                    # Update user details in the database 
                    if user_object.full_name != full_name:
                        user_object.full_name = full_name
                        user_object.save()
                        print_colored("User with ID {}: Full name has been changed to {}".format(user_id, full_name), colors.fg.green)

                    # Establish connection to the database
                    if user_object.username != username:
                        user_object.username = username
                        user_object.save()
                        print_colored("User with ID {}: Username has been changed to {}".format(user_id, username), colors.fg.green)
                        ...
                        try:
                            ...
                            cursor.execute(sql_query, (username, user_id))
                            conn.commit()

                            # Print success message and close the cursor and connection to the database
                            print_colored("User with ID {}: Username updated in baseline_testing database".format(user_id), colors.fg.green)
                        except psycopg2.Error as e:
                            print_colored("Error updating username in baseline_testing database for user with ID {}: {}".format(user_id, e), colors.fg.red)
                        finally:
                            cursor.close()
                            conn.close()
    ...
    # Handle exceptions and close the connection to the database if it is open
    except psycopg2.Error as e:
        print_colored("Error connecting to baseline_testing database: {}".format(e), colors.fg.red)
    ...
    print_summary(success_count, unchanged_count, not_exist_count, skipped_count, len(users))

# Function to print summary of updates and errors
def print_summary(success_count, unchanged_count, not_exist_count, skipped_count, total_users):
    """
    Print summary of updates and errors.
    ...
    if skipped_count > 0:
        print_colored("{} learner(s) were skipped due to missing or empty fields.".format(skipped_count), colors.fg.red)
...
if __name__ == '__main__':
    ...
    if not args.input_file:
        print_colored("Error: Please provide the path to the input file.", colors.fg.red)
        sys.exit(1)
