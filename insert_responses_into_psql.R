...
# helper function to insert the contents of the responses csv file into the responses table on baseline
insert_responses <- function(input_file){
	responses_df <- read.csv(input_file,stringsAsFactors = FALSE)
	dbWriteTable(conn,"responses",responses_df,append = TRUE, row.names = FALSE)
}
